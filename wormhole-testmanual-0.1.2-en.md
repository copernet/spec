Test manual of wormhole version 0.1.2

一、Test environment build

1. Software download：https://github.com/copernet/wormhole/releases/tag/v0.1.2
...
3. Run and data synchronization
   ...
   Run the version 0.1.2 using the following command for the first time：`wormholed -startclean=1 -daemon`

   Use the following command when the software is restarted the next time after the data synchronization is completed for version 0.1.2：`wormholed -daemon`
...
17、Transfer management to the unfreezing address
...
21、Transfer management to the frozen address
...
24、Reboot, no -cleanstart parameter
