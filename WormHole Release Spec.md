...
可以使用以下方案
方案一：使用wormhole提供的钱包完成交易。
方案二：使用独立的钱包完成交易，通过调用服务器端的RPC服务，生成未签名的交易；然后钱包进行签名，向外发送签名后的交易。
...
修改的地方: 
上一版**Wormhole**协议中该类型的交易主要有两种功能

*   转账
*   参与众筹：通过向一个正在的众筹地址转账来实现参与众筹

这一版**Wormhole**协议对该类型进行修改，只存在一种功能
